...
Context.prototype.run = function( block, params, state ) {
    ...
    */
    ...
    this._promise.then( function() {
        console.error('Context.prototype.run clear that._deps_check_timer');
...
Context.prototype._queue_deps_check = function() {
    console.error('Context.prototype._queue_deps_check');
    if ( this._deps_check_timer ) {
        console.error('Context.prototype._queue_deps_check clear that._deps_check_timer');
        clearTimeout( this._deps_check_timer );
    }

    var that = this;
    this._deps_check_timer = setTimeout( function() {
        console.error('that._n_active_blocks', that._n_active_blocks);
        console.error('that._n_blocks', that._n_blocks);
        if ( that._n_active_blocks === 0 && that._n_blocks !== 0 ) {
            //  Есть незавершенные блоки, но активных нет.

            console.error('that._dependent_blocks', that._dependent_blocks);
